...
def compute_speed_of_sound_cramer(
    temperature_c, relative_humidity, atmospheric_pressure_pa, co2_ppm,
):
    """Cramer (1993), Eq. 15/Table III, zero-frequency speed in m/s.

    Temperature is Celsius; RH is percent; pressure is Pa; CO2 is ppm.
    Pressure and CO2 must be supplied explicitly, measured or assumed.
    Validity: 0–30 C, 75–102 kPa, water mole fraction <=0.06, CO2 <=1%.
    Coefficients and humidity conversion corroborated against pyfar 0.8.0.
    DOI: 10.1121/1.405827. This does not model frequency dispersion or wind.
    """
    t, rh, p, co2 = np.broadcast_arrays(
        np.asarray(temperature_c, dtype=float),
        np.asarray(relative_humidity, dtype=float),
        np.asarray(atmospheric_pressure_pa, dtype=float),
        np.asarray(co2_ppm, dtype=float),
    )
    if not all(np.all(np.isfinite(v)) for v in (t, rh, p, co2)):
        raise ValueError("Cramer inputs must be finite")
    if np.any((t < 0) | (t > 30)):
        raise ValueError("Cramer temperature must be 0–30 C")
    if np.any((rh < 0) | (rh > 100)):
        raise ValueError("Relative humidity must be 0–100 percent")
    if np.any((p < 75000) | (p > 102000)):
        raise ValueError("Cramer pressure must be 75000–102000 Pa")
    if np.any((co2 < 0) | (co2 > 10000)):
        raise ValueError("CO2 must be 0–10000 ppm")
    T = t + 273.15
    enhancement = 1.00062 + 3.14e-8*p + 5.6e-7*t**2
    saturation_pa = np.exp(
        1.2811805e-5*T**2 - 1.9509874e-2*T + 34.04926034 - 6.3536311e3/T
    )
    xw = rh/100 * enhancement * saturation_pa/p
    xc = co2*1e-6
    if np.any(xw > 0.06):
        raise ValueError("Cramer water-vapour mole fraction must not exceed 0.06")
    return (
        331.5024 + 0.603055*t - 0.000528*t**2
        + (51.471935 + 0.1495874*t - 0.000782*t**2)*xw
        + (-1.82e-7 + 3.73e-8*t - 2.93e-10*t**2)*p
        + (-85.20931 - 0.228525*t + 5.91e-5*t**2)*xc
        - 2.835149*xw**2 - 2.15e-13*p**2 + 29.179762*xc**2
        + 0.000486*xw*p*xc
    )
