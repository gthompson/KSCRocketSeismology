...
def add_basemap(
    ax,
    provider: str = "esri_streets",
    alpha: float = 0.85,
    zoom: str | int = "auto",
) -> None:
    """Add a Contextily basemap to an axis whose coordinates are lon/lat."""
    providers = {"esri_streets": cx.providers.Esri.WorldStreetMap}
    ...
    provider_max_zoom = int(source.get("max_zoom", 19))
    if isinstance(zoom, (int, np.integer)) and zoom > provider_max_zoom:
        warnings.warn(
            f"Requested basemap zoom {zoom} exceeds the {provider!r} "
            f"provider maximum of {provider_max_zoom}; using "
            f"{provider_max_zoom} instead."
        )
        zoom = provider_max_zoom
    ...
    try:
        ...
        # The sensor geometry remains interpretable without web tiles. This
        # fallback also keeps the figure reproducible when run offline.
        warnings.warn(
            f"Could not retrieve the {provider!r} basemap ({exc}); "
            "continuing with a plain background."
        )
...
def make_figure1(
    ...
    add_basemap(ax, provider="esri_streets", alpha=0.82, zoom="auto")
    ax.set_xlim(lonlim)
    ax.set_ylim(latlim)
    ...
    set_equal_ground_aspect(ax)
    # Keep this metre-scale view within the provider's published maximum
    # zoom. The surveyed sensor positions and connecting geometry are the
    # scientific information; the Esri street-map tiles provide context only.
    add_basemap(ax, provider="esri_streets", alpha=0.68, zoom=19)
    ax.set_xlim(lonlim)
    ax.set_ylim(latlim)
