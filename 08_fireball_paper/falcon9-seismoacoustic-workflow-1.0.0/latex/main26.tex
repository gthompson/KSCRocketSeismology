%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Seismica submission manuscript -- shortened main paper, version 26
% Supporting detail and diagnostic figures are in supplement26.tex.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\makeseistitle{
  ...
  coherence-weighted apparent velocity of 351.98~m~s$^{-1}$, matching the
  meteorological prediction of 350.98~m~s$^{-1}$. Among 46 well-measured
...
The pressure-sensor geometry was reconstructed from cable-end locations
surveyed after sensor removal, so relative errors of approximately 1--2~m are
plausible. Source distances ranged from 1408.3 to 1440.0~m
...
clearest impulses and greater for weak or overlapping signals. Millisecond
digits in stored source times preserve the computational time base and do not
represent millisecond absolute accuracy.

Kennedy Space Center weather-tower observations supplied temperature,
humidity, and wind profiles at seven 5-min epochs from 13:05 to 13:35~UTC.
The USLaunchReport footage provided the public visual and camera-audio record.
Four proprietary SpaceX videos viewed during the 2016 work assisted visual
interpretation but contained no usable audio and supplied no quantitative
result reported here. Publicly archived regional pressure records to 1800~km
...
Still-air sound speed, $c_0$, was calculated from temperature and humidity.
Wind vectors were projected onto the SLC-40--BCHH path and averaged over the
available lower-atmosphere profile. Effective acoustic speed was
\begin{equation}
 c_{\mathrm{eff}}=c_0+\mathbf{u}\boldsymbol{\cdot}\hat{\mathbf{r}}.
 \label{eq:effective_speed}
\end{equation}
The resulting 350.98~m~s$^{-1}$ was rounded to 351~m~s$^{-1}$ for full
source-to-BCHH reduction. Three time coordinates were kept distinct: observed
...
\end{equation}
The catalogue variable \texttt{reduced\_event\_epoch\_s} is therefore an
estimated arrival at DD2, not an origin time.
...
$\geq0.7$. A still stricter array subset additionally required a clear
compression--rarefaction pulse, complete window capture, consistency across
scoring windows, coherence $\geq0.90$, and a non-edge propagation solution.

Continuous Bartlett beamforming provided an independent validation using
0.5-s windows and 2--20~Hz. If $C$ is multichannel coherence for $N=3$
sensors, the array statistic was
\begin{equation}
 F=\frac{(N-1)C}{1-C}.
 \label{eq:f_statistic}
\end{equation}
The 99.5th percentile of the empirical background distribution was $F=3.02$.
...
Mean air temperature was 26.36$\pm0.27\,^{\circ}$C and mean relative humidity
83.9$\pm3.7\%$. The still-air speed was 348.33~m~s$^{-1}$ and the lower-profile
tailwind component was +2.65~m~s$^{-1}$, giving
$c_{\mathrm{eff}}=350.98$~m~s$^{-1}$. The 1419.9-m travel time was 4.046~s,
only 0.031~s shorter than the still-air estimate.
...
criteria. The 51-event high-quality planar subset was reserved for the most
stringent waveform and propagation interpretations. Median robust peak SNR was
...
The 51 high-quality pressure pulses show repeatable compression followed by a
smaller rarefaction, whereas vertical ground velocity has a longer oscillatory
coda (Figure~\ref{fig:waveform_ensemble}). Classical symmetric N-wave
morphology was not observed. Many weak pressure impulses approached the
seismic noise floor.
...
\begin{figure*}[!tbp]
  ...
  waveforms for 51 high-quality events. Pale curves are individual events, the
  dark curve is the pointwise median, and shading spans the 16th--84th
  percentiles.}
  \label{fig:waveform_ensemble}
\end{figure*}

Under the preferred Bartlett configuration, median back azimuth was
...
and 113 events exceeded the background threshold $F=3.02$. The independent
time-domain analysis gave a high-quality coherence-weighted direction of
200.73$^\circ$ and apparent velocity of 351.98~m~s$^{-1}$, within
1~m~s$^{-1}$ of the meteorological prediction. The approximately 1$^\circ$
offset is readily explained by metre-scale array-coordinate uncertainty. The
finite-distance calculation did not resolve range and slightly reduced
coherence; the planar result is preferred.

\begin{figure*}[!tbp]
  \centering
  \IfFileExists{../data/outputs/080_analyze_planar_array_propagation/planar_catalogue_summary_utc_phase.pdf}{%
    \includegraphics[width=0.98\textwidth]{planar_catalogue_summary_utc_phase.pdf}%
  }{%
    \pendingfigure{Three-panel UTC/phase figure showing back azimuth,
    apparent velocity, and Bartlett $F$. Do not substitute the superseded
    four-panel event-number/coherence product.}%
  }
  \caption{Direction, apparent velocity, and Bartlett $F$ through the
  153-transient sequence. Reference lines show the SLC-40 back azimuth,
  meteorological effective speed, and 99.5th-percentile background threshold;
  phase shading and quality coding distinguish robust from weak solutions.}
...
The principal-explosion measurement window contained approximately
$1.89\times10^9$~J under the hemispherical acoustic-radiation model, equivalent
to 453~kg TNT as radiated acoustic energy. At an illustrative 1\% acoustic
efficiency this corresponds to 45~t TNT total yield, but the 0.1--10\%
sensitivity range is 4.5--450~t; these are scenarios, not calibrated blast
yields. Detailed named-window and event-complex energetics are in Supplementary
Section~S5. Because the final cumulative event-complex product has not yet
been re-executed in the standardized workflow, no sequence-wide cumulative
