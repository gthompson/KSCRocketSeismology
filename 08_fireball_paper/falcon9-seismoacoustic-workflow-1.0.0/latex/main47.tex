...
The BCHH record provides a geophysical chronology independent of vehicle
telemetry. Its most direct forensic constraint is the absence of a readily
detectable external acoustic or seismic impulse immediately before the
failure, consistent with the November~2016 forensic report
\citep{thompson_mcnutt_fireball_2016}. The broader analysis was later presented
at the 2017 AGU Fall Meeting \citep{thompson_agu_falcon9_2017}. This result does
not exclude weak, internal, non-impulsive, or poorly coupled processes.

The visual record and source-reduced arrivals support the adopted order of the
initial upper-stage event, principal explosion, payload impact, and later
payload explosion. The movie is anchored to the initial event and therefore
does not independently validate absolute UTC, but it helps distinguish the
major physical stages within a catalogue containing many closely spaced peaks.
SpaceX subsequently attributed the initiating failure to a composite
overwrapped pressure vessel in the second-stage liquid-oxygen tank
...
the reported 93-ms transition from anomalous telemetry to loss of the second
stage.

\subsection{Acoustic propagation and weak-shock interpretation}

Back azimuths locate the signals at SLC-40, while the preferred
principal-explosion apparent speed of 352.5~m~s$^{-1}$ is typical of the
high-quality event population and close to the meteorologically predicted
effective speed. The higher short-window Bartlett result is less stable and is
not evidence that a strongly supersonic shock persisted to BCHH.

Interpreting the measured 1.39-kPa positive overpressure as a normal-shock jump
would imply Mach number about 1.006 and local speed about 353~m~s$^{-1}$ with
the estimated tailwind. The pressure peak alone cannot establish a resolved
shock front because bandwidth, baseline processing, reflection, geometry, and
incidence angle are insufficiently constrained. A simple $1/r$ inward
extrapolation permits a more nonlinear wave near the vehicle, but it is a
sensitivity calculation rather than a source-pressure inversion. We therefore
use 351~m~s$^{-1}$ for common source-time reduction and reserve the shock model
for interpretation (Supplementary Section~S5).

\subsection{Air-to-ground coupling and acoustic energy}

The most transferable quantitative result is the close-range pressure--ground
relation. Across 46 measurement-quality entries, peak-to-peak pressure scaled
nearly linearly with three-component vector PGV: the fitted exponent was 0.962,
and the median proportional relation was
\[
 p_{\mathrm{p2p}}=4.34\times10^5\,\mathrm{PGV},
 \qquad
 \mathrm{PGV}=2.31\times10^{-6}p_{\mathrm{p2p}},
\]
with pressure in Pa and velocity in m~s$^{-1}$. Excluding the principal
measurement group changed the exponent only to 0.943, so the scaling is not
created solely by the largest event. It remains an empirical relation for BCHH,
not a universal conversion.

The H1 analysis adds frequency dependence that the peak-amplitude relation
cannot show. Vertical response per unit pressure was enhanced near 4--5 and
20--24~Hz, with a weaker 13--14-Hz feature; the principal-band H1 magnitude was
approximately $5$--$7\times10^{-6}$~(m~s$^{-1}$)~Pa$^{-1}$. This need not equal
the $2.31\times10^{-6}$ broadband peak-vector conversion because H1 uses
vertical motion and spectral averages within individual frequency bands.
Agreement between event-window and continuous estimates supports a repeatable
local response, comparable in scale to controlled-explosion and volcanic
measurements \citep{novoselov_ground_coupling_2020}. Connecting that response
to material properties would require multiple seismic stations and a shallow
velocity model.

The principal explosion's median hemispherical-equivalent acoustic energy was
$2.2\times10^9$~J, close to the $2.27\times10^9$~J accumulated across the
selected nonoverlapping event complexes. Thus, under the adopted processing and
$2\pi$ radiation model, the principal explosion dominates the selected
acoustic-energy budget. These are finite-passband, model-equivalent acoustic
energies rather than blast yields. Absolute calibration, possible sensor
over-range response, baseline choice, reflection, directivity, and obstacles
remain larger interpretive limits than the precision of the nominal values.
...
No regional counterpart was confirmed, and the flagged regional windows remain
under formal review. Consequently the Falcon~9 event cannot support the
period--yield analysis applied to the regionally detected 2026 New Glenn
static-fire explosion \citep{silber_scamfer_newglenn_2026}. Wind-dependent
spectral filtering provides an additional reason not to transfer regional
period relations without propagation modelling
...
access was restricted. Future monitoring would gain most from several compact
pressure arrays paired with separated three-component seismic stations. That
geometry would add triangulation and path sampling, distinguish direct ground
waves from local air-to-ground conversion, and test whether the BCHH coupling
relation transfers between sites. Continuous recording remains essential for
background characterization and post-accident chronology.
...
calibration uncertainty affects all absolute pressure-dependent results, while
baseline, geometry, propagation, and possible over-range nonlinearity add
uncertainties not included in that percentage. Nearby-launch raw-count checks
support relative DD2/DD3 stability, but do not identify DD2's nominal head or
establish an absolute calibration. These limitations preserve the relative timing,
direction, and scaling results but require caution in absolute energy, yield,
and seismic-phase interpretations.
...
20~$\mu$Pa and its median hemispherical-equivalent acoustic-energy estimate was
