...
$4.34\times10^5$~Pa~m$^{-1}$~s. These resampling ranges exclude calibration
and dependence between nominally separate measurement groups.
...
\begin{figure*}[!tbp]
  ...
  relation; the annotation gives the conditional overlap-group-bootstrap
  percentile range. The dashed unit-slope reference represents direct
...
The six-channel review, consistent with the November~2016 forensic report
\citep{thompson_mcnutt_fireball_2016}, argues against a readily detectable
external acoustic or seismic impulse immediately before the failure. The
broader forensic analysis was later presented at the 2017 AGU Fall Meeting
\citep{thompson_agu_falcon9_2017}. These observations do not exclude weak,
internal, non-impulsive, or poorly coupled processes.

Later visual features and source-reduced receiver arrivals are consistent
with the adopted chronology, array direction and apparent velocity. The
movie is anchored to the initial upper-stage event, so agreement at that
anchor does not independently validate absolute source UTC. The
...
The published 2017 AGU summary documents the same 153-transient sequence and
dominant principal event \citep{thompson_agu_falcon9_2017}; the present
pressure--Mach calculation develops the propagation assessment beyond that
historical summary.

If the measured median positive overpressure of 1.39~kPa is interpreted as
a normal-shock pressure jump, it corresponds to local Mach number about 1.006
and speed approximately 353~m~s$^{-1}$ with the estimated tailwind. Bandwidth,
baseline processing and reflection prevent the peak alone from establishing
that a resolved shock front was recorded. Geometry uncertainty and possible
incidence-angle effects likewise prevent a few-m~s$^{-1}$ trace-speed excess
from resolving this small Mach excess. A simple $1/r$ inward extrapolation gives a larger
...
beyond this estimate (Supplementary Section~S1). Baseline-window sensitivity
also affects integrated pressure-squared measurements and is not represented
by the calibration estimate. The relative factors have not
yet been independently reassessed using launches bracketing the accident. The regression
bootstrap retains overlapping measurement rows together but does not account
for dependence between groups or systematic calibration/model errors. Regional
